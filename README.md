...
The single-file `index.html` embeds all 72 images for offline browsing. Its course links open the hosted recordings. A complete local course folder also works: open `courses.html` with its sidecar files, 24 MP3 files and all credited images beside it. Evidence links still require internet. Feel & Experience includes a separately controlled Olympic forest drone. It follows Play, Pause, seeking and course completion; Learn remains voice-only. The 142-second stereo soundscape loops with an eight-second blended seam. It combines local ACE-Step music with authored synthesized rain, canopy and distant-water textures, rather than claiming an on-location field recording. The background starts quietly at 35%, with independent mute and volume controls that remember your choice. `forest-soundscape.json` records the production settings and credits; `forest-audio-data.js` supports offline playback.
